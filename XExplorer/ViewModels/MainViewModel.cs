...
using XExplorer.Utils;
using XExplorer.Views;
...
public partial class MainViewModel : ObservableObject
{
	...
	public async Task FolderAsync(object param)
	{
		try
		{
			var obj = await Dialog.Show<InputPwd>()
				.Initialize<InputPwdViewModel>(vm => vm.Password = this.Password)
				.GetResultAsync<string>();

			Growl.Success($"Input password: {obj}");
	...
	[RelayCommand]
	public async Task ExtractAsync(object param)
	{
		try
		{
			var passwords = this.GetPasswords();
			var rootDir = this.SelectedDir;
			var output = rootDir;

			await Task.Run(() =>
			{
				string[] zipFiles = Directory.GetFiles(rootDir, "*.zip");
				string[] rarFiles = Directory.GetFiles(rootDir, "*.rar");
				string[] allFiles = zipFiles.Concat(rarFiles).ToArray();

				Parallel.ForEach(allFiles, file =>
				{
					try
					{
						var msg = ZipUtils.Extract(file, output, passwords);

						if (string.IsNullOrWhiteSpace(msg))
						{
							Log.Information($"{file} extracted.");
							Growl.Info($"{file} extracted.");
							File.Delete(file);
						}
						else
						{
					...
					{
						Log.Error($"file extract error : {e}");
						File.Delete(file);
			...
			Growl.Success($"The decompression task is completed.");
		}
		catch (Exception ex)
		{
			Log.Error(ex, $"{MethodBase.GetCurrentMethod().Name} Is Error");
	...
	[RelayCommand]
	public async Task AddPwdAsync(object param)
	{
		try
		{
			var obj = await Dialog.Show<InputPwd>()
			...
			await this.AddPwdAsync(obj);

			Growl.Success($"Input password: {obj} added");
		}
		catch (Exception ex)
		{
			Log.Error(ex, $"{MethodBase.GetCurrentMethod().Name} Is Error");
