...
partial class MainViewModel
{
	...
	/// <returns></returns>
	public async Task<List<string>> LoadPics(string path, int count = 50)
	{
		...
		return await Task.Run(() =>
		{
			if (!(this.allImages?.Any() ?? false))
			{
				...
				var filtedPics = new ConcurrentBag<FileInfo>();
				var dirInfo = new DirectoryInfo(this.SelectedDir); 

				Parallel.ForEach(exts, (ext) =>
				{
					var files = dirInfo.EnumerateFiles(ext, SearchOption.AllDirectories)
						.Where(file => file.Length > oneMbSize)
						.Select(file => file);
					foreach (var file in files) 
						filtedPics.Add(file); 
				});

				this.allImages = filtedPics.ToList();
			}
		...
		using (var cts = new CancellationTokenSource(TimeSpan.FromMinutes(5)))
		{
			try
			{
				await Task.Run(async () =>
				{
					...
					Parallel.ForEach(videos, item => { item.Status = Convert.ToDecimal(File.Exists(item.VideoPath)); });
	...
	/// <summary>
	/// Find similar images based on a sample image and a list of image files.
	/// </summary>
	/// <param name="samplePath">The path of the sample image.</param>
	/// <param name="files">The list of image files to compare with the sample image.</param>
	/// <returns>The list of matched image file paths.</returns>
	private List<string> FindSimilarImages(string samplePath, List<string> files)
	{
		var matchedImages = new List<string>();

		// 加载样本图片
		using var sampleImage = new Mat(samplePath, ImreadModes.Color);
		using var sift = new SIFT();
		var sampleKeyPoints = new VectorOfKeyPoint();
		var sampleDescriptors = new Mat();
		sift.DetectAndCompute(sampleImage, null, sampleKeyPoints, sampleDescriptors, false);

		foreach (var file in files)
		{
			using var image = new Mat(file, ImreadModes.Color);
			var keyPoints = new VectorOfKeyPoint();
			var descriptors = new Mat();
			sift.DetectAndCompute(image, null, keyPoints, descriptors, false);

			// 使用BF匹配器进行特征匹配
			using var bfMatcher = new BFMatcher(DistanceType.L2);
			var matches = new VectorOfVectorOfDMatch();
			bfMatcher.KnnMatch(sampleDescriptors, descriptors, matches, k: 2);
			...
			// 根据好的匹配数量决定是否将图片加入结果列表
			if (goodMatches > 10) // 可根据需要调整阈值
			{
				matchedImages.Add(file);
			}
		}
